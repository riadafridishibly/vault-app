...
// Mutate calls f(ctx, m).
func (f PasswordItemFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.PasswordItemMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.PasswordItemMutation", m)
	}
	return f(ctx, mv)
}
